...
bool Mesh::LoadMesh(const std::string &fileLocation, const std::string &fileName) {
	ClearData();
...
bool Mesh::InitFromData(const std::vector<VertexFormat> &vertices, const std::vector<unsigned int> &indices) {
...
bool Mesh::InitFromData(const std::vector<VertexFormat> &vertices, const std::vector<unsigned int> &indices, Texture2D *texture) {
	this->vertices = vertices;
	this->indices = indices;
	...
	meshEntries[meshEntries.size() - 1].materialIndex = materials.size() - 1;
...
bool Mesh::InitFromData(const std::vector<glm::vec3> &positions, const std::vector<glm::vec3> &normals, const std::vector<unsigned int> &indices) {
...
void Mesh::CopyAnimations(const aiScene *pScene) {
	...
	for (unsigned int i = 0; i < pScene->mNumAnimations; i++) {
		...
		for (int j = 0; j < anim[i]->mNumChannels; j++) {
			...
			memcpy(anim[i]->mChannels[j]->mPositionKeys,
				   pScene->mAnimations[i]->mChannels[j]->mPositionKeys,
			...
			memcpy(anim[i]->mChannels[j]->mRotationKeys,
				   pScene->mAnimations[i]->mChannels[j]->mRotationKeys,
			...
			memcpy(anim[i]->mChannels[j]->mScalingKeys, pScene->mAnimations[i]->mChannels[j]->mScalingKeys, anim[i]->mChannels[j]->mNumScalingKeys * sizeof(aiVectorKey));
		}
		...
		for (int j = 0; j < anim[i]->mNumMeshChannels; j++) {
			...
			anim[i]->mMeshChannels[j]->mKeys =
				new aiMeshKey(pScene->mAnimations[i]->mMeshChannels[j]->mKeys->mTime, pScene->mAnimations[i]->mMeshChannels[j]->mKeys->mValue);
		}
	}
}
...
void Mesh::UseMaterials(bool value) { useMaterial = value; }

GLuint Mesh::GetVAO() const { return buffers->m_VAO; }

size_t Mesh::IndicesCount() const {
	size_t count = 0;
	for (const auto &entry : meshEntries) {
		count += entry.nrIndices;
	}
	return count;
}
...
void Mesh::Render() const {
	...
	for (unsigned int i = 0; i < meshEntries.size(); i++) {
		...
		glDrawElementsBaseVertex(glDrawMode,
